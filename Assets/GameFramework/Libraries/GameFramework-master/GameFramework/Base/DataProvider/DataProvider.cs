...
namespace GameFramework
{
    ...
    internal sealed class DataProvider<T> : IDataProvider<T>
    {
        ...
        public void ReadData(string dataAssetName, bool isBinary = false)
        {
        ...
        public void ReadData(string dataAssetName, int priority, bool isBinary = false)
        {
        ...
        public void ReadData(string dataAssetName, object userData, bool isBinary = false)
        {
        ...
        /// <param name="priority">加载数据资源的优先级。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void ReadData(string dataAssetName, int priority, object userData, bool isBinary = false)
        {
            if (aas_ResourceManager == null)
            {
            ...
            aas_ResourceManager.LoadAssetAsync<UnityEngine.TextAsset>(dataAssetName).Completed += (h) => {
                if(h.Status==UnityEngine.ResourceManagement.AsyncOperations.AsyncOperationStatus.Succeeded && h.Result != null)
                {
                    if (isBinary)
                        m_LoadBinaryCallbacks.LoadBinarySuccessCallback(dataAssetName, h.Result.bytes, 0, userData);
                    else
                        m_LoadAssetCallbacks.LoadAssetSuccessCallback(dataAssetName, h.Result.text, 0, userData);
                }
                else
                {
                    if (isBinary)
                        m_LoadBinaryCallbacks.LoadBinaryFailureCallback(dataAssetName, LoadResourceStatus.AssetError, h.OperationException.Message, userData);
                    else
                        m_LoadAssetCallbacks.LoadAssetFailureCallback(dataAssetName, LoadResourceStatus.AssetError, h.OperationException.Message, userData);
                }
            };
        }
        /// <summary>
        ...
        internal void SetResourceManager(IAddressableResourceManager resourceManager)
        {
            ...
            aas_ResourceManager = resourceManager;
        }
