...
namespace GameFramework
{
    ...
    internal sealed partial class EventPool<T> where T : BaseEventArgs
    {
        ...
        {
            Dictionary<int, List<EventHandler<T>>> dic = new Dictionary<int, List<EventHandler<T>>>();
            foreach (var value in m_EventHandlers)
            {
                int id = value.Key;
                ...
                if (m_EventHandlers.TryGetValue(id, out range))
                {
                    ...
                    {
                        LinkedListNode<EventHandler<T>> temp = current.Next != range.Terminal ? current.Next : null;
                        if (current.Value!=null)
                        {
                            
                            List<EventHandler<T>> hTemp;
                            if (!dic.TryGetValue(id,out hTemp))
                            {
                                hTemp = new List<EventHandler<T>>();
                                dic.Add(id,hTemp);
                            }
                            hTemp.Add(current.Value);
            ...
            foreach (KeyValuePair<int, List<EventHandler<T>>> kv in dic)
            {
                foreach (EventHandler<T> h in kv.Value)
                {
                    Unsubscribe(kv.Key,h);
                }
            }
        }
        /// <summary>
