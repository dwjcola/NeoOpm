...
namespace GameFramework.Entity
{
    ...
    internal sealed partial class EntityManager : GameFrameworkModule, IEntityManager
    {
        ...
        private  IAddressableResourceManager aas_ResourceManager;
        ...
        public void SetResourceManager(IAddressableResourceManager resourceManager)
        {
            ...
            aas_ResourceManager = resourceManager??GameFrameworkEntry.GetModule<IAddressableResourceManager>();
        }
        ...
        /// <param name="entityGroupName">实体组名称。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void ShowEntitySync(int entityId, string entityAssetName, string entityGroupName, object userData)
        {
            ShowEntitySync(entityId, entityAssetName, entityGroupName, Constant.DefaultPriority, userData);
        }
        ...
        public void ShowEntity(int entityId, string entityAssetName, string entityGroupName, int priority, object userData)
        {
            ...
            if (entityInstanceObject == null)
            {
                ...
                aas_ResourceManager.LoadAsset<UnityEngine.GameObject>(entityAssetName, m_LoadAssetCallbacks, ShowEntityInfo.Create(serialId, entityId, entityGroup, userData));
        ...
        /// <param name="priority">加载实体资源的优先级。</param>
        /// <param name="userData">用户自定义数据。</param>
        public void ShowEntitySync(int entityId, string entityAssetName, string entityGroupName, int priority, object userData)
        {
            if (m_EntityHelper == null)
            {
                throw new GameFrameworkException("You must set entity helper first.");
            ...
            if (entityInstanceObject == null)
            {
                int serialId = ++m_Serial;
                m_EntitiesBeingLoaded.Add(entityId, serialId);
                var asset =aas_ResourceManager.LoadAssetSync<UnityEngine.GameObject>(entityAssetName);
                if (asset == null)
                {
                    m_LoadAssetCallbacks.LoadAssetFailureCallback?.Invoke(entityAssetName, LoadResourceStatus.AssetError, "Can not load this asset from asset database.", ShowEntityInfo.Create(serialId, entityId, entityGroup, userData));
                }
                else
                {
                    m_LoadAssetCallbacks.LoadAssetSuccessCallback?.Invoke(entityAssetName, asset, 0, ShowEntityInfo.Create(serialId, entityId, entityGroup, userData));
                }
                return;
            }
        ...
        public void RemoveAllEntity()
        {
            ...
            foreach (KeyValuePair<string, EntityGroup> entityGroup in m_EntityGroups)
            {
                entityGroup.Value.ReleaseAllUnused();
                entityGroup.Value.Release();
            }
            m_RecycleQueue.Clear();
        }
