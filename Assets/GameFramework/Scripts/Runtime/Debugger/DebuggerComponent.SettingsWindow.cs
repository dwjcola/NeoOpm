    ...
    public sealed partial class DebuggerComponent : GameFrameworkComponent
    {
        private sealed class SettingsWindow : ScrollableDebuggerWindowBase
        {
            ...
            protected override void OnDrawScrollableWindow()
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    GUILayout.Label($"FontSize:{(int)mFontSize}", GUILayout.Width(60f));
                    if (GUILayout.RepeatButton("-", GUILayout.Width(30f)))
                    {
                        mFontSize--;
                    }
                    mFontSize = GUILayout.HorizontalSlider(mFontSize, 10, 60);
                    if (GUILayout.RepeatButton("+", GUILayout.Width(30f)))
                    {
                        mFontSize++;
                    }
                    mFontSize = Mathf.Clamp(mFontSize, 10, 60);
                    if (((int)mFontSize) != DebuggerComponent.style.fontSize)
                    {
                        DebuggerComponent.style.fontSize=(int)mFontSize;
                    }
                }
                GUILayout.EndHorizontal();
            }
