﻿Shader "OPM/ToonShader_rim"
{
	Properties
	{
        ...
        _MaskDataTex1("遮罩数据图1 用于存储渲染数据  Rim Mask (R)  Outline Mask (G)  Rim Mask2 (B)  Outline Mask Z Offset (A)", 2D) = "white" {}
		//rim
		_RimColor2("亮Rim Color", Color) = (0.8,0.8,0.8,0.6)
		_RimColor("暗Rim Color", Color) = (0.8,0.8,0.8,0.6)
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				float4 maskDataTex1 = tex2Dlod(_MaskDataTex1, float4(v.uv,0,0));
				vPos.xyz += normalize(vPos.xyz)* (1-maskDataTex1.a);
				
				float3 vNormal = mul((float3x3)UNITY_MATRIX_IT_MV,v.tangent);
				o.pos = mul(UNITY_MATRIX_P,vPos);
				float2 offset = TransformViewToProjection(vNormal.xy);
				offset += offset * cameraDis;// * maskDataTex1.b;
				o.pos.xy +=offset * _Outline*maskDataTex1.g;
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				/*float maskTex = tex2D(_MaskDataTex1, i.uv).g;
				clip(maskTex - 0.01);*/
				float maxChan = max(max(mainTex.r, mainTex.g), mainTex.b);
				float4 newMapColor = mainTex;
				maxChan -= (1.0 / 255.0);
				...
				fixed4 col =saturate( _OutlineColor*float4(0.6 * newMapColor.rgb * mainTex.rgb, mainTex.a));
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed3 _main = tex2D(_MainTex, i.uv);
				...
				half rimD = rim * saturate(-NdotLrim);
				half rimL = rim * saturate(NdotLrim);
				//rim *= saturate(-NdotLrim);
				fixed3 maskTex = tex2D(_MaskDataTex1, i.uv);
				fixed3 rimColor = saturate(saturate( rimD*_RimColor)*maskTex.r + saturate( rimL*_RimColor2)*maskTex.b);
				...
				half shade = smoothstep(0, (1 - ao) + _shadeSoft, NdotL);
				shade = lerp(shade * ao, 1.0, shade);
				half3 shadeColor = lerp(lerp(shadowColorfinal.rgb * 0.8, shadowColorfinal.rgb, ao), _main, shade );

                half shade2 = smoothstep(0, (1 - ao) + _shadeSoft, NdotL + _towShadowRange);
				shade2 = lerp(shade2 * ao, 1.0, shade2);
				half3 shadeColor2 = lerp(lerp(twoshadowColorfinal.rgb * 0.8, twoshadowColorfinal.rgb, ao), shadeColor, lerp(1, shade2 , sssTex.a));
