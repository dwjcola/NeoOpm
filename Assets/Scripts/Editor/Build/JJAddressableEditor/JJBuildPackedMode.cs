...
namespace UnityEditor.AddressableAssets.Build.DataBuilders
{
    ...
    /// </summary>
    [CreateAssetMenu(fileName = "JJBuildPackedMode.asset", menuName = "Addressables/Content Builders/JJBuildPackedMode")]
    public class JJBuildPackedMode : BuildScriptBase
    {
        /// <inheritdoc />
        ...
        {
            var platform = PlatformMappingService.GetPlatform();
            string platformPath = platform.ToString();
            if (platform == AddressablesPlatform.Unknown)
            {
#if UNITY_EDITOR
        ...
        protected virtual TResult DoBuild<TResult>(AddressablesDataBuilderInput builderInput, AddressableAssetsBuildContext aaContext) where TResult : IDataBuilderResult
        {
            ...
            var tempPath = Path.GetDirectoryName(Application.dataPath) + "/" + LibraryPath + GetPlatformPathSubFolder() + "/addressables_content_state.bin";
            ...
            if (m_AllBundleInputDefs.Count > 0)
            {
                ...
                if (ProjectConfigData.GenerateBuildLayout)
                {
                    ...
                    var field = typeof(BuildLayoutGenerationTask).GetField("m_BundleNameRemap", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
                    field?.SetValue(buildLayoutTask, bundleRenameMap);
            ...
            if (extractData.BuildCache != null && builderInput.PreviousContentState == null)
            {
                ...
                if (ContentUpdateScript.SaveContentState(aaContext.locations, tempPath, allEntries, extractData.DependencyData, playerBuildVersion, remoteCatalogLoadPath, carryOverCachedState))
                {
                    ...
                    {
                        if (!IsVCAssetOpenForEdit(contentStatePath))
        ...
        internal bool CreateCatalogFiles(string jsonText, AddressablesDataBuilderInput builderInput, AddressableAssetsBuildContext aaContext)
        {
            ...
            {
                dependencyHashes = CreateRemoteCatalog(jsonText, aaContext.runtimeData.CatalogLocations, aaContext.Settings, builderInput, new NeoOPM.ProviderLoadRequestOptions() { IgnoreFailures = true });
            }
            ...
            IBundleWriteData writeData, Dictionary<string, ContentCatalogDataEntry> locationIdToCatalogEntryMap)
        {
            foreach (var loc in assetEntries)
            {
                ...
                if (AssetDatabase.IsValidFolder(loc.AssetPath))
                {

                    List<AddressableAssetEntry> entries = new List<AddressableAssetEntry>();
                    processedEntry.GatherAllAssets(entries, false, true, true);
                    if (entries.Count > 0)
                        processedEntry = entries[0];
                }
        ...
        internal static void GenerateBuildInputDefinitions(List<AddressableAssetEntry> allEntries, List<AssetBundleBuild> buildInputDefs, string groupGuid, string address, bool ignoreUnsupportedFilesInBuild)
        {
            ...
            {
                if (!ThrowExceptionIfInvalidFiletypeOrAddress(e, ignoreUnsupportedFilesInBuild))
                    continue;
        ...
        private static bool ThrowExceptionIfInvalidFiletypeOrAddress(AddressableAssetEntry entry, bool ignoreUnsupportedFilesInBuild)
        {
            ...
            var MainAssetType = AssetDatabase.GetMainAssetTypeAtPath(entry.AssetPath) ?? typeof(object);
            if (MainAssetType == typeof(DefaultAsset) && !AssetDatabase.IsValidFolder(entry.AssetPath))
            {
        ...
        internal static AssetBundleBuild GenerateBuildInputDefinition(List<AddressableAssetEntry> assets, string name)
        {
            ...
            assetsInputDef.assetBundleName = name.ToLower().Replace(" ", "").Replace('\\', '/').Replace("//", "/").Replace("/", "_");
        ...
        static string[] CreateRemoteCatalog(string jsonText, List<ResourceLocationData> locations, AddressableAssetSettings aaSettings, AddressablesDataBuilderInput builderInput, NeoOPM.ProviderLoadRequestOptions catalogLoadOptions)
        {
            string[] dependencyHashes = null;
        ...
        {
            if (/*schema.BundleMode == BundledAssetGroupSchema.BundlePackingMode.PackTogether*/true)
            {
